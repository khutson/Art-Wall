...
from artwall import create_app, db
from artwall.models import User, Role

app = create_app()
migrate = Migrate(app, db, directory="artwall/migrations")
...
def init_db(migration):
    ...
    if migration:
        ...
        db.create_all()
        db.session.commit()
        cfg = alembic.config.Config("artwall/migrations/alembic.ini")
